...
#define OCTAVES 5
        ...
        void main()
        {
            ...
            vec2 coord = gl_FragCoord.xy / (resolution * 1.);
        
            vec3 result = pos.xyz * 0.05;

        vec3 nebulaColor1 = hsv2rgb(vec3(0.0157, 0.3059, 0.0667));
        vec3 nebulaColor2 = hsv2rgb(vec3(0.9541, 0.6745, 0.6549));
            
            result += stars(coord + u_mouse.xy * 0.001, 4., 0.1, 2.) * vec3(.74, .74, .74);
            result += stars(coord + u_mouse.xy * 0.0005, 8., 0.05, 1.) * vec3(.97, .74, .74);
